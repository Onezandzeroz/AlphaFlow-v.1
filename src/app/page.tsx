...
import { AcceptInvitationScreen } from '@/components/auth/accept-invitation-screen';
...
export default function Home() {
  ...
  // After registration the user sees a "check email" screen inside the
  // RegisterForm — no login session is created.  This callback is a no-op
    // that simply keeps the user on the unauthenticated login screen.
  const handleRegisterSuccess = useCallback(() => {
    // Do nothing — RegisterForm handles its own "check your email" UI.
  }, []);
  ...
  // ─── Email verification flow (when user clicks link in email) ───
  // Render a dedicated verification screen when ?verify=token is present.
  // This works for both authenticated and unauthenticated users.
  // We read from window.location.search directly to avoid needing useSearchParams
  // (which requires a Suspense boundary in Next.js).
  // Parse query params from window.location.search (no useSearchParams needed)
  const queryParams = typeof window !== 'undefined'
    ? new URLSearchParams(window.location.search)
    : null;

  const verifyToken = queryParams?.get('verify') ?? null;
  if (verifyToken) {
    return <VerifyEmailScreen token={verifyToken} />;
  }

  // ─── Invitation acceptance flow (when invitee clicks link in email) ───
  const inviteToken = queryParams?.get('invite') ?? null;
  if (inviteToken) {
