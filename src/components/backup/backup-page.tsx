...
export function BackupPage({ user }: BackupPageProps) {
  ...
  // Fetch scheduler status (auto-refreshes at same interval as backups)
  useEffect(() => {
    ...
    const interval = setInterval(fetchSchedulerStatus, POLL_INTERVAL_MS);
  ...
  const getAutoCountForType = useCallback((backupType: string): { count: number; lastAt: string | null } => {
    ...
    return { count: entry?.count ?? 0, lastAt: entry?.lastAt ?? null };
  }, [schedulerStatus?.autoBackupCounts]);
