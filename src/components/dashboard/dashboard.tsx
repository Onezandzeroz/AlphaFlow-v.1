...
export function Dashboard({ user, onNavigate, onboardingStepJustDone, onOnboardingStepDoneConsumed }: DashboardProps) {
  ...
  // Auto-dismiss onboarding when step 2 is just completed
  useEffect(() => {
    if (onboardingStepJustDone === 2 && completedSteps === onboardingSteps.length && !isLoading) {
      // Step 2 just done and both steps complete — show onboarding briefly then dismiss
  ...
  }, [onboardingStepJustDone, completedSteps, onboardingSteps.length, isLoading, onOnboardingStepDoneConsumed]);
  ...
  const quickActions = [
    {
      ...
      bgColor: 'bg-[#f0fdf9] dark:bg-[#1a2e2b]',
    ...
    },
    {
      ...
      bgColor: 'bg-[#faf5ee] dark:bg-[#302a22]',
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0d9488] via-[#14b8a6] to-[#5eead4] dark:from-[#0f766e] dark:via-[#0d6058] dark:to-[#0d9488] min-h-[180px] sm:min-h-[220px] text-white">
            ...
            <div className="relative flex flex-col sm:flex-row items-start sm:items-stretch justify-between gap-4 p-6 sm:p-8">
              ...
              <div className="shrink-0 flex items-end justify-center sm:justify-end pb-0">
                <Button
                  variant="outline"
                  className="gap-2 px-5 py-2.5 border-2 border-white/40 bg-white/15 backdrop-blur-md hover:bg-white/25 hover:border-white/60 text-white font-semibold rounded-xl transition-all duration-300 shadow-lg shadow-black/20"
        ...
        action={
          <div className="flex items-center gap-1.5">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-sm gap-2 transition-all duration-200">
                ...
                {quickActions.map((action) => {
                  ...
                  return (
                    <button
                      ...
                      `}
                    >
                      <div className={`h-10 w-10 rounded-xl ${action.bgColor} flex items-center justify-center shrink-0 transition-all duration-300 group-hover:bg-white/20 dark:group-hover:bg-white/10`}
                ...
                {vatBreakdown.length > 0 ? (
                  ...
                  <div className="h-64 min-h-[200px] sm:min-h-[250px] flex flex-col items-center justify-center gap-3">
                    <div className="h-14 w-14 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                      <Calculator className="h-6 w-6 text-gray-300 dark:text-gray-600" />
                    </div>
                ...
                {journalEntries.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 gap-3">
                    <div className="h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                      <PenLine className="h-5 w-5 text-gray-300 dark:text-gray-600" />
                    </div>
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {journalEntries.map((entry) => {
                      ...
                      return (
                        <div
                          key={entry.id}
                          className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 cursor-pointer group hover:shadow-sm"
                          onClick={() => onNavigate?.('journal')}
                        >
              ...
              <CardContent className="p-4 sm:p-6">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  ...
                  {journalEntries.length === 0 && transactions.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-8 gap-3">
                      <div className="h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                        <Activity className="h-5 w-5 text-gray-300 dark:text-gray-600" />
                      </div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">
