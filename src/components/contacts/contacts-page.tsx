      ...
      <PageHeader
        ...
        action={
          <Button
            ...
            className="bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-sm font-medium gap-2 transition-all"
          >
      ...
      <Card className="stat-card">
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {/* Type filters - mobile dropdown / desktop pills */}
            ...
            >
              <div className="flex flex-wrap gap-2">
                {([
                  { value: 'ALL', label: isDanish ? 'Alle' : 'All' },
                  { value: 'CUSTOMER', label: isDanish ? 'Kunder' : 'Customers' },
                  { value: 'SUPPLIER', label: isDanish ? 'Leverandører' : 'Suppliers' },
                  { value: 'BOTH', label: isDanish ? 'Begge' : 'Both' },
                ] as const).map((pill) => (
                  <button
                    key={pill.value}
                ...
                {hasActiveFilters && (
                  <button
                    ...
                    className="hidden lg:inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 transition-colors"
                  >
