...
│  │ Stores    │  │ State    │  │ (50 Radix + TW4 based)     │     │
│  │ (auth,    │  │ (fetch)  │  │                            │     │
│  │ sidebar,  │  │          │  │ Recharts                   │     │
│  │ language, │  │          │  │                            │     │
...
| **Components** | [shadcn/ui](https://ui.shadcn.com/) | new-york | 50 Radix UI-based components (48 standard + 2 custom responsive) |
...
The application exposes **71 API route files** under `/api/`. All mutating endpoints require session-based authentication and generate audit log entries. The system uses `tenantFilter()` from `rbac.ts` for multi-tenant data isolation on all queries.
...
│   │   └── ui/                    # 50 shadcn/ui components (48 standard + 2 custom responsive)
│   ├── lib/
...
│   │   ├── clear-demo-data.ts      # Demo data cleanup utility
│   │   ├── logger.ts              # Structured logging (dev=debug, prod=warn+)
│   │   ├── port-utils.ts          # Port availability scanning
│   │   ├── use-swipe-navigation.ts # Swipe gesture hook for mobile page transitions
...
Yes, switch between Danish and English at any time via the language button in the sidebar. The choice is saved automatically. The system has ~360+ translation keys covering all UI text.
...
| API Endpoints | 70 endpoints | **71 route files** (verified count of all `route.ts` files) |
...
| Translation Coverage | Not quantified | **~360+ keys** covering all UI text |
...
*Last updated: April 2025 — Based on full source code audit of commit from `main` branch. Verified: 71 API routes, 50 UI components, 34 lib modules, 22 Prisma models, 15 enums, 21 SPA views.*
